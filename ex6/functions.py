...
def dataSet3Params(X, y, Xval, yval, C_range=(-3,3), sig_range=(-3,3)):
    C_range = np.logspace(C_range[0],C_range[1],12)
    sig_range = np.logspace(sig_range[0],sig_range[1],12)
    err = 1000
    C_best = -1
    sig_best = -1
    for i, C in enumerate(C_range):
        for j, sig in enumerate(sig_range):
            clf = svmTrain(X, y, C, kernel='rbf', gamma=sig)
            ...
            if error < err:
                ...
                final_clf = clf
    if C_best == -1:
        raise ValueError('C and sigma arent improving')

    return C_best, sig_best, final_clf

def getVocabList(path):
    vocabList = np.genfromtxt(path, dtype=object)
    return list(vocabList[:, 1].astype(str))

# Not gonna lie i just copied this one straight up
class PorterStemmer:
    """
...
def processEmail(email_contents, vocab_path, verbose=True):    
    vocabList = getVocabList(vocab_path)
    word_indices = []
    ...
    for word in email_contents:
        ...
        try:
            ...
            # handles words not in the vocab list
            pass
...
def emailFeatures(word_indices, vocab_count):
    x = np.zeros(vocab_count)
